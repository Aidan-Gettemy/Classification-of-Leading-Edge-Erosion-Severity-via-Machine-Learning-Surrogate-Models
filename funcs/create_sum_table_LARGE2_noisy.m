function [status, length, stats] = create_sum_table_LARGE2_noisy(directory,in_table,loc,DT,noise_entries,noise_level,bias_level)
...
%
    ...
    for i = 1:numel(variablenames)
        ...
        x = x.Variables;
        if ismember(i,noise_entries)
            disp('Noise Entry')
            bias = bias_level * abs(mean(x)) * randn();
            sigma_noise = noise_level * std(x);
            x = x + bias + sigma_noise * randn(size(x));
        end
    ...
    output_ID = directory + "/SensorData_SumLargeNoisy.txt";
