...
M = readtable("Exp6_inTable.txt");
variableList = {"mean","sd","skew","kurt"};

% Create ExperimentResultTable
expTab = combineResultsLarge(M,StatusFileID,variableList);
saveTabID = ExperimentID+"/LARGE2ExperimentResultTable1_600.txt";
writetable(expTab,saveTabID);

%% Repeat, but calculate the features using noisy data for all inputs
DT = 1/160;
trans = 60;
noise_level = 0.1;
bias_level  = 0.1;
...
saveTabID = ExperimentID+"/LARGE2noisyExperimentResultTable1_600.txt";
writetable(expTab,saveTabID);
%%
T1 =      readtable("Data\Exp6\LARGE2ExperimentResultTable1_600.txt");
T2 = readtable("Data\Exp6\LARGE2noisyExperimentResultTable1_600.txt");
%%
figure
...
saveTabID = ExperimentID+"/LARGE3ExperimentResultTable1_600.txt";
writetable(expTab,saveTabID);
