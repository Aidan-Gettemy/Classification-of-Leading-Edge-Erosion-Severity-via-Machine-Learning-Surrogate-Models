...
print('-dpng',saveID)
%% Add sensor noise to the following outputs: 40-B1N6Cd, 34-B1N6Cl

% Add sensor noise to the specified outputs
noiseLevel = 0.05; % Define the noise level
for i = 1:1%numel(batch)
    dataTab = batch{i}{1}; % Extract the data table
    
    noise_level = 0.05;   % 5% noise
    x = dataTab.GenPwr;

    sigma_noise = noise_level * std(x);
    x_noisy = x + sigma_noise * randn(size(x));
    
    % batch{i}{1} = dataTab; % Update the batch with noisy data
    plot(dataTab.GenPwr)
    hold on
    plot(x_noisy)
end
